...
export const handler = async (event) => {
  ...
  try {
    ...
    const content_length = content.trim().split(/\s+/).length;
    const has_emojis = /[\u{1F300}-\u{1F9FF}]/u.test(content);
    const has_hashtags = /#\w+/.test(content);
    const has_cta = /comment|share|tag|save|follow|like|click/i.test(content);
...
=== CONTENT METRICS ===
- Word count: ${content_length}
- Character count: ${content.length}
- Sentences: ${sentences}
- Has emojis: ${has_emojis}
...
1. QUALITY SCORE (0-100) - Be STRICT:
   ...
   STRICT RULES:
   - If ${content_length} < 10 words: score MUST be 20-35
   - If ${content_length} < 30 words: score MUST be 35-50
   - If ${content_length} < 80 words: score MUST be 50-70
...

{
  ...
  "suggestions": [
    "Expand to 100-150 words - current ${content_length} words is too short. Add: personal story, statistics, and emotional appeal",
    ...
    "${has_emojis ? 'Add 3-4 MORE emojis for emphasis: Use 🔥 for excitement, 💡 for ideas, ✨ for magic, 🚀 for growth, 🎯 for goals' : 'Add 5-8 strategic emojis: 🔥💡✨🚀💪🎯👏❤️ to boost engagement by 30%'}",
    ...
    "${has_cta ? 'Strengthen CTA with urgency: Change to Comment NOW, Share if you agree RIGHT NOW, Tag 3 friends TODAY, Save this before it is gone' : 'Add powerful CTA: End with Comment your thoughts below, Double-tap if you agree, Share this with someone who needs it, Save for later'}",
  ...
  "engagementPrediction": "${content_length < 15 ? 'Low' : content_length < 60 ? 'Medium' : 'High'}",
  ...
  "issues": [
    "Length: Only ${content_length} words - needs 80-150 words for optimal ${platform} performance (current is ${content_length < 50 ? 'way too short' : 'too short'})",
    "${has_emojis ? 'Emoji usage: Has emojis but needs 3-4 more for emphasis and visual breaks' : 'Missing emojis: Add 5-8 emojis (🔥💡✨🚀💪) to increase engagement by 30%'}",
    "${has_hashtags ? 'Hashtags: Present but needs 15-20 more for maximum discoverability' : 'No hashtags: Add 20-30 relevant hashtags for reach and discoverability'}",
    "Value proposition: Unclear what specific benefit or transformation readers will get - add concrete outcomes",
    "Hook weakness: First sentence is ${content_length < 5 ? 'too short and' : ''} generic - needs curiosity gap, question, or bold claim",
    "${has_cta ? 'CTA present but weak: Add urgency words like NOW, TODAY, LIMITED, BEFORE IT IS GONE' : 'Missing CTA: Add clear action - Comment, Share, Save, Tag, Follow, Click link'}"
  ],
